...
pub fn init_app() {
    ...
    log_msg("init_app() started.");

    log_msg("Disabling owner validation globally...");
    unsafe {
    ...
    log_msg("Owner validation disabled.");
    ...
    for dir in cert_dirs {
        if Path::new(dir).exists() {
            log_msg(&format!("Probing cert directory: {}", dir));
            unsafe {
                if git2::opts::set_ssl_cert_dir(dir).is_ok() {
                    log_msg(&format!("set_ssl_cert_dir set to: {}", dir));
                    break;
                }
            }
        }
